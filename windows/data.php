...
function handleCSVImport() {
    ...
    try {
        ...
        while (($data = fgetcsv($handle)) !== false) {
            if (count($data) < 11) {
                $errors[] = "Row has insufficient data (expected 11 columns)";
                continue;
            }
            
            // Map CSV data based on header: 
            // Name,Index No,Email,Phone,Academic Year,Dues Paid,Receipt No,Programme of Study,payment date,password,position
            ...
            $dues_paid = sanitizeInput($data[5] ?? '0');
            $receipt_no = sanitizeInput($data[6] ?? '');
            $programme = sanitizeInput($data[7] ?? '');
            $payment_date = sanitizeInput($data[8] ?? '');
            $csv_password = $data[9] ?? '';
            $position = strtolower(sanitizeInput($data[10] ?? 'student'));
            ...
            if (!$existing_user) {
                ...
                $role_id = $role_data ? $role_data['role_id'] : 4; // Default to student role
                ...
                $stmt->execute([$index_no, $hashed_password, $email, $role_id]);
                $user_id = $pdo->lastInsertId();
            } else {
                $user_id = $existing_user['user_id'];
            }
            
            // 2. Handle Student Data (ONLY if position is 'student')
            if ($position === 'student') {
                ...
                if ($stmt->fetch()) {
                    ...
                    // Split name into first_name and surname (assuming "Surname , Firstname" or similar)
                    $name_parts = explode(',', $name);
                    $ln = trim($name_parts[0] ?? '');
                    $fn = trim($name_parts[1] ?? '');
                    
                    // Insert student
                    $stmt = $pdo->prepare("INSERT INTO students (index_no, first_name, surname, email, phone, academic_year, programme_id, position, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
                    $stmt->execute([$index_no, $fn, $ln, $email, $phone, $academic_year, $programme_id, $position, $user_id]);
                    
                    // 3. Handle Initial Payment (if provided in CSV)
                    if (!empty($dues_paid) && (float)$dues_paid > 0 && !empty($receipt_no)) {
                        $student_id = $pdo->lastInsertId();
                        // Check if receipt already exists
                        $stmt = $pdo->prepare("SELECT payment_id FROM payments WHERE receipt_no = ?");
                        $stmt->execute([$receipt_no]);
                        if (!$stmt->fetch()) {
                            // Format date if needed (CSV is DD.MM.YYYY, SQL needs YYYY-MM-DD or similar)
                            $date_obj = date_create_from_format('d.m.Y', $payment_date);
                            ...
                            $stmt->execute([$student_id, (float)$dues_paid, $receipt_no, $sql_date, $academic_year, $_SESSION['user_id']]);
                        }
                    }
